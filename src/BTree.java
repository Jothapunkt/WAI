...
public class BTree<Key extends Comparable<Key>, Value>  {
    ...
    public Entry[] getN(Key key, int n) {
        if (key == null) throw new IllegalArgumentException("argument to get() is null");
        return searchN(root, key, height, n);
    }

    private Entry[] searchN(Node x, Key key, int ht, int n) {
        Entry[] children = x.children;
        ...
        // internal node
            ...
            for (int j = 0; j < x.m; j++) {
                if (j+1 == x.m || less(key, children[j+1].key))
                    return searchN(children[j].next, key, ht-1, n);
            }
        }
        return null;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < iterations; i++){
            st.getN(rnd.nextInt(collectionsize -neigbours), neigbours);
        }
